...
def inject_s2_temporal_overlap(folds):
    ...
    for f in folds:
        c = dict(f)
        # Teste começa no meio do treino
...
def run_injection_scenario(name, description, contaminated_folds, validator):
    ...
    print(f"\n{'='*60}")
    print(f"  Cenário {name}: {description}")
    print(f"{'='*60}")

    # Mostrar primeiro fold contaminado para diagnóstico
    ...
    try:
        ...
        print(f"  FALHA: Gate NÃO detectou a violação!")
        return False
    except ValueError as e:
        print(f"  Gate bloqueou execução com diagnóstico:")
        print(f"    {str(e)[:200]}")
        return True
...
def run_s4_empirical_comparison():
    ...
    print(f"\n{'='*60}")
    print(f"  Cenário S4: Experimento empírico — leakage vs clean")
    print(f"{'='*60}")

    # Gerar dados com estrutura temporal que evidencia leakage.
    # O cenário simula concept drift: a relação features->target
    # muda ao longo do tempo (regime shifts a cada ~8 anos).
    # Walk-forward captura essa dificuldade; k-fold naive esconde-a
    # ao misturar dados de diferentes regimes no mesmo fold.
    n_countries = 32
    ...
    for c in range(n_countries):
        ...
        for y_idx, y in enumerate(years):
            ...
            # Feature que "vaza" informação futura:
            # média móvel centrada que usa valores de t-1 a t+1
    ...
    # ---- Configuração CONTAMINADA: K-fold naive + future features ----
...
def main():
    ...
    print("=" * 60)
    print("  TESTE DE INJEÇÃO DE LEAKAGE")
    print("  Validação negativa do gate anti-leakage")
    print("=" * 60)

    # Gerar folds válidos como baseline
    valid_folds = generate_valid_folds()
    validator = TemporalValidator(min_gap_years=2)

    # Verificar que folds válidos passam
    ...
    print(f"\n{'='*60}")
    print(f"  SUMÁRIO: {'TODOS os cenários detectados' if all_detected else 'FALHA em algum cenário'}")
    print(f"  Baseline válido: {baseline_ok}")
    for name, s in results['scenarios'].items():
        status = "DETECTADO" if s['leakage_detected'] else "FALHOU"
        print(f"  {name}: {status} — {s['description']}")

    # S4: Experimento empírico (se não --quick)
    if not args.quick:
    ...
    if baseline_ok and all_detected:
        print("\n  RESULTADO FINAL: SUCESSO — Gate anti-leakage funcional")
        sys.exit(0)
    else:
